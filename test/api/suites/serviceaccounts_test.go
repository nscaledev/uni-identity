		...
		Describe("Given valid organization", func() {
			It("should return all service accounts in the organization with complete metadata", func() {
				...
				if len(serviceAccounts) == 0 {
					GinkgoWriter.Printf("Organization has no service accounts (valid state)\n")
					return
				}
				...
				for _, sa := range serviceAccounts {
					...
					if len(sa.Spec.GroupIDs) > 0 {
						Expect(sa.Spec.GroupIDs).NotTo(BeEmpty(),
							"Service account should be a member of at least one group")
					}
