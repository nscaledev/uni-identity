...
| **B: Distributed** | Reshaped: a per-enclave identity authorization unit evaluating replicated state, not a sidecar per service (`distributed-pdp-design.md`). | needs a narrow specification §10.1 amendment ("single *policy authority*"). | **approved 2026-07-28**, superseding the deferred sidecar shape ([D5](#open-decisions)). |
...
## 6. Distributed deployment is designed, and reshaped

**Superseded by `distributed-pdp-design.md`, approved 2026-07-28.** B is no longer
deferred, and it is no longer a Cerbos sidecar per service. It is a per-enclave identity
authorization unit that evaluates centrally-authored policy against enclave-local
replicated state. Read that design for the topology, the trust argument and the rollout.
...
- **D5, Platform specification amendment. ✅ Settled by
  `distributed-pdp-design.md`** (approved 2026-07-28). Distribution is a per-enclave
  authorization unit that generates policy locally, so no per-service distribution
  mechanism is needed. A narrow specification §10.1 amendment is still required, and it is
  an external dependency on `nscaledev/uni-specifications`. Not needed for the central
  compatible cutover.
