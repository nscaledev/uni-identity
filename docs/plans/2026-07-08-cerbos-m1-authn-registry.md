...
- [ ] **A3 — Reconciling controller.** Watch `Role` CRDs → run A2 → write to the sidecar's policy volume/ConfigMap → hot-reload. Handles GitOps *and* manually-applied `Role` CRs. *Deliverable:* applying/editing a `Role` CR updates Cerbos policy at runtime. *Test:* integration — create a custom `Role`, assert a decision that depends on it flips. *Depends:* A2.
...
- [ ] **A8 — Internal `/authorization/check` + `remote` path.** OpenAPI spec entry (`x-hidden`, mTLS) + handler + `remote` authorizer decision call. *Deliverable:* a downstream service obtains a decision from identity. *Test:* integration via the typed client over mTLS. *Depends:* A5.
