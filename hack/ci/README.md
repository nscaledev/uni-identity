...
| `test-values.yaml` | Helm value overrides for CI: pre-configures the `ci-fixtures` system account, runs the server in shadow authorization mode (feeding `divergence-gate`), and injects placeholder open-vocabulary `example:*`/`sample:*` roles so the whole generate→compile→publish pipeline is exercised on them |
