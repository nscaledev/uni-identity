...
func (i *JWTIssuer) DecodeJWEToken(ctx context.Context, tokenString string, claims any, tokenType TokenType) error {
	// UNI access tokens are signed (ES512) then encrypted under a symmetric key
	// the issuer alone holds (A256GCMKW), so the algorithms are pinned here.
	nestedToken, err := jwt.ParseSignedAndEncrypted(tokenString,
		[]jose.KeyAlgorithm{jose.A256GCMKW},
		[]jose.ContentEncryption{jose.A256GCM},
		[]jose.SignatureAlgorithm{jose.ES512},
	)
	if err != nil {
