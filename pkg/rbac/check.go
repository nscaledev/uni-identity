...
// This file is the Cerbos decision API: resolve the
// subject's bindings, build one CheckResources request, and map the response
// to allow/deny.  Impersonated requests are served by the impersonation
// dual-check — two AND-ed single-principal evaluations, the impersonated
// principal and the acting service (decideImpersonated) — replacing the legacy
// confused-deputy ACL intersection.  The PolicyDecisionPoint seam is the PDP
// CLIENT boundary: only the local in-process Cerbos client satisfies it.  The
// remote decision path does NOT plug a "remote PDP" in here — its POST
// /authorization/check endpoint (pkg/handler) is a REMOTE ENTRY into THIS
// decision layer: a downstream service without a sidecar calls identity, whose
// handler runs CheckMany against the SAME local PDP client.  (A remote
// transport BELOW decide() would still need to read identity's authorization
// resources (the Group/Role/Project/Organization CRDs) to resolve bindings —
// Kubernetes access a downstream service does not have — which is exactly what
// the endpoint centralizes; see decision_engine.go for why the Allow*-facade
// remote-provider is a separate designed follow-up.)  Decision audit logging
// and metrics live in decision_log.go, hooked at the CheckMany choke point
// below so every consumer — Check, allowCoarse, and the remote
// /authorization/check handler alike — is observed, pre-PDP fail-closed denials
// included.
...
var (
	...
	// unknown or empty (see impersonationTypeGate).  NARROWED with the
	// impersonation dual-check — valid User/Service impersonations are served
	// by the dual check (decideImpersonated) and no longer land here.  The
	// sentinel (and its "impersonation" decision-record/counter class) is
	// retained under the narrowed meaning: the class vocabulary is closed and
	// renames are breaking (see decision_log.go and pkg/rbac/README.md).
...
// where the remote decision path lives — the /authorization/check endpoint is
// a remote ENTRY into this layer (its handler calls CheckMany, which uses this
// seam's local client), not an alternative implementation of this interface.
...
// impersonation dual-check) — returning per-check verdicts in request order.
// The error taxonomy matches Check, except plain policy denies are the false
...
// predicate, type-gated) are the impersonation dual-check.  The decision
// records' policy correlate is the policy-store hash
// (recordDecisions/shadowCompare read it from r.policyHasher), so the raw PDP
// response is no longer threaded out.
...
// the impersonated principal and the acting service (the impersonation
// dual-check, replacing the legacy confused-deputy intersection
// getSystemAccountACL performs).
//
// EQUIVALENCE (the proven dual-check grounding): the legacy path answers from
...
// (resource, action) equals the legacy intersect verdict, given the decision
// layer's single-principal parity.  The service side inherits global→org→project
