...
// This file is the grantability cross-parity integration test (run it with
...
// open-vocabulary roles (example:*/sample:*) — so a change to the generator or the
...
func xparityRoleCatalogue(t *testing.T) []unikornv1.Role {
	...
	for _, role := range parityRoles() {
		if role.Name != parityRoleExampleOperator && role.Name != parityRoleSampleProjectViewer {
			continue
		}
...
// TestGrantabilityCrossParity is the grantability cross-parity deliverable: for
// every role, the generated Cerbos policy must grant a holder of that role
// EXACTLY the scopes the grant-guard (AllowRole) trusts it to — no more (the
// security-critical direction), no less.  Both engines evaluate the SAME roles
// over the SAME fixture data; any per-probe divergence is a
// grant-guard/enforcement split to escalate, not to special-case.
