...
// These tests pin the shadow-mode comparator: in shadow mode every
...
// taxonomy is load-bearing for the cutover gate: verdict disagreements
...
// The exact log messages are the shadow-phase observability contract (the
...
func TestShadowDivergenceCerbosAllows(t *testing.T) {
	...
	// divergence record must carry as its policy correlate.
...
func TestShadowEvaluationFailureIsNotDivergence(t *testing.T) {
	...
	// The taxonomy split this test pins is what keeps the zero-divergence
...
func TestShadowImpersonatedComparisonRuns(t *testing.T) {
	...
	// dual-check, the comparison covers impersonated traffic: the legacy
	// intersection verdict against the AND-ed dual-check verdict — two PDP
	// calls per shadowed check.
	ctx := shadowContext(t, capture, engine, globalACL("candy", openapi.Read))
