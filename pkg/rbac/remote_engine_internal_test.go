...
// remoteEngineFromContext directly -- it has no caller yet (the remote dispatch
// fork's dispatchCoarse is the first), so an external rbac_test file could not
...
// through the context.  The remote-engine context seam only seeds and reads
// the context -- dispatch (the remote dispatch fork) is what actually calls an
// engine -- so these methods are never invoked; they exist solely to satisfy
// the interface.
