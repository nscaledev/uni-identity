...
type fakePDP struct {
	...
	// steps, when non-empty, are consumed one per call — the impersonation
	// dual-check tests need per-side verdicts and failures.  response/err
	// answer every call otherwise.
	steps []fakePDPStep
...
func TestCheckImpersonatedDualCheck(t *testing.T) {
	...
	// the impersonation dual-check: two AND-ed single-principal evaluations over
	// the identical resource and action.
