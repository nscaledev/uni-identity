...
// This file is the shadow-mode comparator: with --authorization-engine
...
//
// The log taxonomy is load-bearing for the cutover gate:
//
...
//     message strings.  This is the policy-parity signal the cutover gate reads.
...
// the same sink as the decision records (decision_log.go).  logr has no
// warn level; both record classes emit at Info so they are unconditionally
// visible, which the cutover gate requires.
//
...
// A kind cut over to Cerbos (its modeForKind is cerbos, the strangle-by-kind
// cutover switch) is therefore NOT shadowed under a shadow baseline — it is
// authoritative-served through engineForDispatch instead, exactly right.
// Impersonated requests are compared too, as of the impersonation dual-check:
// the legacy intersection verdict against the AND-ed dual-check verdict — both
// single booleans, so the comparator needs no structural change.
...
func (r *RBAC) shadowCompare(ctx context.Context, resource Resource, operation openapi.AclOperation, legacyAllowed bool) {
	...
	case err == nil, goerrors.Is(err, ErrPolicyDenied):
		...
		// none is configured (the policy-store-hash correlate, replacing the
		// empty PDP version/scope echo).
	...
	default:
		...
		// unclassified error): infra signal, NEVER divergence — the
