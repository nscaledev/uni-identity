...
// This file is the dual-path dispatch seam: the Allow* facade can serve
...
// the request context.  The legacy path is retained verbatim — the shadow-mode
// comparator (shadow.go) runs it live alongside the Cerbos path, and the
// per-kind cutover (modeForKind) makes Cerbos authoritative one kind at a time
// — and it is removed only once the legacy path is retired, after the
// strangle-by-kind cutover.
...
// EngineMode selects which engine serves Allow* decisions.  The
// strangle-by-kind cutover adds per-kind authoritative flags on top.
...
// strangle-by-kind cutover switch), so it is served by the PDP even under a
// legacy or shadow global mode; every other kind follows the global mode.
// Matching is exact-string on the endpoint — one endpoint is strangled at a
// time (no wildcards in M1).  With an empty cutover set (the default)
// modeForKind == mode() for every kind, so dispatch is byte-identical to the
// pre-cutover global behaviour, and clearing a kind from the set reverts it
...
// per-kind cutover (modeForKind).  Impersonated requests are served too,
// via the impersonation dual-check (decideImpersonated in check.go); an invalid
...
// the shadow-mode comparator and the decision metrics.
//
...
// (the remote /authorization/check handler) never call it and are never
// cached. Only explicit policy denials are cached; allows and transient
// failures are not, so neither policy reload lag nor a PDP outage can poison a
// later request. Cache hits skip the decision log and the PDP-served counter (a hit
// is not a new PDP decision) but are counted in the coarse-cache hit/miss
// counter for cache observability.
func (r *RBAC) allowCoarse(ctx context.Context, resource Resource, operation openapi.AclOperation) error {
	...
	// impersonated principal type must be refused BEFORE any cache lookup,
	// independently of the key's principal-type dimension.
