...
// This file is the shared fixture for the bindings resolver unit tests and
...
const (
	...
	// parityAdminIssuer is the src_iss the parity fixtures resolve to: they set
	// no SrcIss, so the resolver defaults to the UNI sentinel.
	parityAdminIssuer = idconstants.UNISentinel
	...
	// populated for external tokens, so both cases need a real issuer.
	parityExternalIssuer = "https://idp.example.com/"
	parityExternalGroup  = "parity-idp-group"
	...
	// Open-vocabulary additions: frank holds example:*/sample:* roles
	// (placeholders for out-of-repo services) via parityGroupOpenVocab,
	// which project Z links for the project-scope grants (a NEW project so
	// the existing X/Y cells stay untouched).  See parityRoles for details.
	parityRoleExampleOperator     = "prole-example-operator"
	parityRoleSampleProjectViewer = "prole-sample-project-viewer"
	parityGroupOpenVocab          = "parity-group-openvocab"
	parityProjectZ                = "parity-project-z"
	parityFrankSubject            = "parity-frank@example.com"

	// Impersonation additions: a SECOND registered system account
...
type parityFixture struct {
	...
	// options is the configuration rbac.New consumed, kept so withOptions can
	// build a variant instance over the same data.
...
func parityRoles() []unikornv1.Role {
	...
	return []unikornv1.Role{
		...
		// Open-vocabulary shapes for out-of-repo services (placeholder
		// endpoints, this being a public repo): parityRoleExampleOperator
		// mirrors an example:* org-scoped role, and
		// parityRoleSampleProjectViewer mirrors a sample:* viewer role with an
		// org-scope read plus project-scope read-only grants.
		role(parityRoleExampleOperator, unikornv1.RoleScopes{
		...
		role(parityRoleSampleProjectViewer, unikornv1.RoleScopes{
			Organization: []unikornv1.RoleScope{scope("sample:assets", unikornv1.Read)},
			Project: []unikornv1.RoleScope{
				scope("sample:records", unikornv1.Read),
				scope("sample:groups", unikornv1.Read),
				scope("sample:sets", unikornv1.Read),
				scope("sample:nodes", unikornv1.Read),
			},
		}),
		// The impersonating service's role: GLOBAL scopes only, like
...
func parityGroups() []unikornv1.Group {
	...
	return []unikornv1.Group{
		...
		// Frank's open-vocabulary bindings: example:* at org scope,
		// sample:* mixed org/project — project Z links this group.
		group(parityOrgANS, parityGroupOpenVocab, unikornv1.GroupSpec{
			RoleIDs:  []string{parityRoleExampleOperator, parityRoleSampleProjectViewer},
...
// withOptions builds a second RBAC over the same fixture data with the options
// mutated.  Global role bindings are resolved once in rbac.New, so a test that
// configures them needs its own instance rather than the shared fx.rbac.
func (f *parityFixture) withOptions(mutate func(*rbac.Options)) *rbac.RBAC {
	options := *f.options
	mutate(&options)

	return rbac.New(f.client, parityNamespace, &options)
}

// parityExternalUserInfo builds the authorization info an EXTERNAL IdP token
// yields: the issuer plus the groups claim, the two inputs group bindings and
// wildcard subject bindings resolve against.
func parityExternalUserInfo(subject string, orgIDs ...string) *authorization.Info {
	info := parityUserInfo(subject, orgIDs...)
