...
// These tests pin the dual-path dispatch seam: the Allow* facade serves
...
// impersonation dual-check).  Every other combination — crucially the total
// absence of an engine, which is every downstream service, NewSuperContext and
// every pre-existing test — takes the legacy ACL walk unchanged.  That
...
func TestAllowDispatchDenyMapping(t *testing.T) {
	...
	// for the shadow-mode comparator and the decision metrics.
...
func TestAllowDispatchImpersonatedServesDualCheck(t *testing.T) {
	...
	// impersonation marker, and a non-empty actor.  Since the impersonation
	// dual-check, cerbos mode serves impersonated traffic via the dual check:
	// two PDP calls per decision (the impersonated principal AND the acting
	// service).
...
func TestAllowRoleStaysLegacy(t *testing.T) {
	...
	// by design (grantability cross-parity covers it), nested scope checks
	// included.
	pdp := &capturePDP{}
	engine := newDispatchEngine(t, rbac.EngineCerbos, pdp)
	...
	require.Zero(t, pdp.calls, "AllowRole must stay legacy-only (grantability parity is covered separately)")
}
