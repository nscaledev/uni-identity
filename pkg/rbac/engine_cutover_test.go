...
// These tests pin the strangle-by-kind cutover DISPATCH behaviour on top of
...
// log capture (logCapture, decisionlog_test.go) as the dual-path dispatch and
// shadow-mode tests.

// newCutoverEngine builds an RBAC over the parity fixture with the given global
// mode, PDP, and per-kind Cerbos-authoritative cutover set — the
// strangle-by-kind switch.  It mirrors newDispatchEngine but sets
// CerbosAuthoritativeKinds, the only field the strangle-by-kind cutover adds
...
func TestCutoverKindProjectScopeCreateIsAuthoritative(t *testing.T) {
	t.Parallel()
	...
	pdp := &capturePDP{}
	engine := newCutoverEngine(t, rbac.EngineLegacy, pdp, cutoverKind)
	ctx := rbac.NewEngineContext(rbac.NewContext(aliceContext(t), globalACL(cutoverKind, openapi.Create)), engine)

	err := rbac.AllowProjectScopeCreate(ctx, nil, cutoverKind, openapi.Create, organizationID, projectID)
	require.True(t, coreerrors.IsForbidden(err), "the cut-over kind must serve the PDP deny, not the legacy create allow")
	require.ErrorIs(t, err, rbac.ErrPolicyDenied)
	require.Equal(t, 1, pdp.calls)
}
...
// TestCutoverKindProjectScopeCreateValidatesTheBodyProject pins the half of the
// authoritative create that the deny tests cannot reach. An engine verdict does
// not report which scope granted it, so the body-supplied project has to be
// validated live even where the legacy walk trusted the caller outright. Every
// other create test passes a nil client and asserts a deny, so none of them ever
// reaches validation, and the fail-open direction (skipping the check after an
// allow) would pass the suite.
func TestCutoverKindProjectScopeCreateValidatesTheBodyProject(t *testing.T) {
	t.Parallel()

	const cutoverKind = "identity:groups"

	notFound := &openapi.GetApiV1OrganizationsOrganizationIDProjectsProjectIDResponse{
		HTTPResponse: &http.Response{StatusCode: http.StatusNotFound},
	}

	t.Run("AProjectThatDoesNotExistFailsAfterTheAllow", func(t *testing.T) {
		t.Parallel()

		engine := newCutoverEngine(t, rbac.EngineLegacy, &capturePDP{allow: true}, cutoverKind)
		ctx := rbac.NewEngineContext(rbac.NewContext(aliceContext(t), globalACL(cutoverKind, openapi.Create)), engine)

		client := openapiMock.NewMockClientWithResponsesInterface(gomock.NewController(t))
		client.EXPECT().
			GetApiV1OrganizationsOrganizationIDProjectsProjectIDWithResponse(gomock.Any(),
				ids.MustParseOrganizationID(organizationID), ids.MustParseProjectID(projectID)).
			Return(notFound, nil)

		err := rbac.AllowProjectScopeCreate(ctx, client, cutoverKind, openapi.Create, organizationID, projectID)
		require.True(t, coreerrors.IsHTTPNotFound(err), "an authoritative allow must not take the body project on trust")
	})

	t.Run("AMissingClientIsAServerFaultNotADenial", func(t *testing.T) {
		t.Parallel()

		// A consumer that seeds an engine but no identity client cannot
		// validate, and the authoritative path always validates. That is a
		// wiring fault: it must fail closed, but as a server error, because a
		// 401 would send every client into a token-refresh retry loop.
		engine := newCutoverEngine(t, rbac.EngineLegacy, &capturePDP{allow: true}, cutoverKind)
		ctx := rbac.NewEngineContext(rbac.NewContext(aliceContext(t), globalACL(cutoverKind, openapi.Create)), engine)

		err := rbac.AllowProjectScopeCreate(ctx, nil, cutoverKind, openapi.Create, organizationID, projectID)
		require.ErrorIs(t, err, rbac.ErrNoIdentityClient)
		require.False(t, coreerrors.IsAccessDenied(err), "a wiring fault must not be reported as an access denial")
	})
}
