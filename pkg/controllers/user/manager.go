...
import (
	...
	"github.com/unikorn-cloud/identity/pkg/provisioners/user"
...
func (*Factory) Reconciler(options *options.Options, controllerOptions coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	return coremanager.NewReconciler(options, controllerOptions, manager, user.New)
}

// RegisterWatches adds any watches that would trigger a reconcile.
func (*Factory) RegisterWatches(manager manager.Manager, controller controller.Controller) error {
	if err := controller.Watch(source.Kind(manager.GetCache(), &unikornv1.User{}, &handler.TypedEnqueueRequestForObject[*unikornv1.User]{}, &predicate.TypedGenerationChangedPredicate[*unikornv1.User]{})); err != nil {
		return err
	}
