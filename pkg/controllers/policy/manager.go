...
func (f *Factory) Reconciler(options *options.Options, _ coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	...
	// GetEventRecorder is the newer accessor, but it returns an
	...
	// reconciler and its tests.  Deferred to its own change.  The pinned
	// controller-runtime carries no deprecation notice on the call below, so
	// no lint directive is needed.
