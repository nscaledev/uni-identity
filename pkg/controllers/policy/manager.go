...
func (f *Factory) Reconciler(options *options.Options, _ coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	...
	// GetEventRecorder is the replacement accessor, but it returns an
	// events.EventRecorder whose Eventf takes a related object and an action
	// verb, so moving to it changes the emitted event shape and touches the
	// reconciler and its tests.  The pinned controller-runtime (v0.23.3)
	// deprecates GetEventRecorderFor in favour of it.  Deferred to its own
	// change.
	return policycontroller.New(f.client, manager.GetEventRecorderFor(constants.Application), options.Namespace, f.controllerOptions, gate) //nolint:staticcheck // SA1019: deliberate, see the comment above.
}
