...
import (
	...
	"github.com/unikorn-cloud/identity/pkg/provisioners/auth0user"
...
func (*Factory) Reconciler(options *options.Options, controllerOptions coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	return coremanager.NewReconciler(options, controllerOptions, manager, auth0user.New)
}

// RegisterWatches adds any watches that would trigger a reconcile.
func (*Factory) RegisterWatches(manager manager.Manager, controller controller.Controller) error {
	if err := controller.Watch(source.Kind(manager.GetCache(), &unikornv1.Auth0User{}, &handler.TypedEnqueueRequestForObject[*unikornv1.Auth0User]{}, &predicate.TypedGenerationChangedPredicate[*unikornv1.Auth0User]{})); err != nil {
		return err
	}
