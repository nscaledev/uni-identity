...
import (
	...
	"github.com/unikorn-cloud/identity/pkg/provisioners/organizationuser"
...
func (*Factory) Reconciler(options *options.Options, controllerOptions coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	return coremanager.NewReconciler(options, controllerOptions, manager, organizationuser.New)
}

// RegisterWatches adds any watches that would trigger a reconcile.
func (*Factory) RegisterWatches(manager manager.Manager, controller controller.Controller) error {
	if err := controller.Watch(source.Kind(manager.GetCache(), &unikornv1.OrganizationUser{}, &handler.TypedEnqueueRequestForObject[*unikornv1.OrganizationUser]{}, &predicate.TypedGenerationChangedPredicate[*unikornv1.OrganizationUser]{})); err != nil {
		return err
	}
