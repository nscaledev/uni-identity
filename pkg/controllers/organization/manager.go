...
func (f *Factory) Reconciler(options *options.Options, controllerOptions coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	f.reconciler = coremanager.NewReconciler(options, controllerOptions, manager, organization.New)
...
func (f *Factory) RegisterWatches(manager manager.Manager, controller controller.Controller) error {
	// An organization provisions once and is done, and this is its only watch, so
	// a restart has nothing to catch up on.  GenerationUnprocessed therefore
	// drops the start up create event for any organization already reconciled at
	// its current generation, rather than re-provisioning the whole fleet.  It
	...
	predicates := predicate.And(
		&predicate.TypedGenerationChangedPredicate[*unikornv1.Organization]{},
		coremanager.GenerationUnprocessed[*unikornv1.Organization](f.reconciler),
	)

	if err := controller.Watch(source.Kind(manager.GetCache(), &unikornv1.Organization{}, &handler.TypedEnqueueRequestForObject[*unikornv1.Organization]{}, predicates)); err != nil {
		return err
	}
