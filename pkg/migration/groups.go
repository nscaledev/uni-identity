...
var errMigrationFailed = errors.New("failed to migrate all records")
var errMalformedOrgUser = errors.New("OrganizationUser is not well-formed")
...
func MigrateGroupsToSubjects(ctx context.Context, c client.Client, identityNamespace string, issuer common.IssuerValue) error {
	...
	for i := range groups.Items {
		...
		subjects := make([]unikornv1.GroupSubject, 0, len(group.Spec.UserIDs))

		// Resolve each UserID to a subject
		for _, userID := range group.Spec.UserIDs {
			subject, err := resolveUserIDToSubject(ctx, c, identityNamespace, group.Namespace, userID)
			if err != nil {
	...
	if errorCount > 0 {
		return fmt.Errorf("migration completed with %d errors: %w", errorCount, errMigrationFailed)
	}
...
func resolveUserIDToSubject(ctx context.Context, c client.Client, globalNamespace, groupNamespace, userID string) (unikornv1.GroupSubject, error) {
	var result unikornv1.GroupSubject
	...
	if err := c.Get(ctx, orgUserKey, orgUser); err != nil {
		return result, fmt.Errorf("OrganizationUser not found: %w", err)
	}

	// Get the user label that references the global User resource
	globalUserID, ok := orgUser.Labels[constants.UserLabel]
	if !ok {
		return result, fmt.Errorf("missing user label: %w", errMalformedOrgUser)
	}
	...
	if err := c.Get(ctx, userKey, user); err != nil {
		return result, fmt.Errorf("resource User %#v not found: %w", userKey, err)
	}

	result.ID = user.Spec.Subject
	result.Email = user.Spec.Subject
