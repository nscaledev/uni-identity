...
func deduplicateGroupSubjects(in []unikornv1.GroupSubject) []unikornv1.GroupSubject {
	if in == nil {
		return nil
	}

	out := make([]unikornv1.GroupSubject, 0, len(in))
	seen := make(map[string]struct{}, len(in))
...
func (c *Client) populateSubjectsAndUserIDs(ctx context.Context, organization *organizations.Meta, in *openapi.GroupWrite) ([]string, []unikornv1.GroupSubject, error) {
	...
	var normalizedSubjects []unikornv1.GroupSubject
	if in.Spec.Subjects != nil {
		normalizedSubjects = deduplicateGroupSubjects(generateSubjects(*in.Spec.Subjects))
	}

	hasSubjects := len(normalizedSubjects) > 0
...
func (c *Client) validateRoleIDs(ctx context.Context, organizationID ids.OrganizationID, roleIDs, currentRoleIDs []string) ([]string, error) {
	...
	for _, roleID := range normalizedRoleIDs {
		...
		if err := rbac.AllowRole(ctx, &resource, organizationID); err != nil {
			return nil, errors.HTTPForbidden(fmt.Sprintf("role %q (%s) cannot be granted: the caller does not hold all its permissions", common.RoleDisplayName(&resource), roleID)).WithError(err)
		}
	}
...
// hasMemberAdditions reports whether the update puts a user, subject or
// service account on the group that was not already a member.  Subjects are
// compared whole: populateSubjectsAndUserIDs has already normalized both
// membership representations by the time this runs.
func hasMemberAdditions(current, required *unikornv1.Group) bool {
	for _, userID := range required.Spec.UserIDs {
		if !slices.Contains(current.Spec.UserIDs, userID) {
			return true
		}
	}

	for _, subject := range required.Spec.Subjects {
		if !slices.Contains(current.Spec.Subjects, subject) {
			return true
		}
	}

	for _, serviceAccountID := range required.Spec.ServiceAccountIDs {
...
func (c *Client) validateMemberAdditions(ctx context.Context, organizationID ids.OrganizationID, current, required *unikornv1.Group) error {
	if !hasMemberAdditions(current, required) {
		return nil
	}
...
func (c *Client) Update(ctx context.Context, organizationID ids.OrganizationID, groupID string, request *openapi.GroupWrite) error {
	...
	if err := c.validateMemberAdditions(ctx, organizationID, current, required); err != nil {
		return err
	}
