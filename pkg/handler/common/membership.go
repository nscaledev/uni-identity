...
// from a group is likewise allowed — role removals are not grant-checked at
// all, as validateRoleIDs in pkg/handler/groups explains.  The asymmetry is
// deliberate: skipping an unresolvable role errs towards less authority on a
// removal and towards more on an addition, so only the removal side is safe
// to skip.
