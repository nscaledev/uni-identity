...
// authorization decisions from identity.
//
...
// serves identity's OWN Allow* facade), just like open-vocabulary roles: it is
// a Cerbos-only path with no legacy twin.
//
...
// any X-Principal/X-Impersonate principal + marker, and therefore the
// impersonation dual check, and the decision-log records and metrics, all apply
// unchanged.
...
func allowServiceAccountOrSelfAccess(ctx context.Context, operation openapi.AclOperation, organizationID ids.OrganizationID, serviceAccountID string) error {
	...
	if info.ServiceAccount && (serviceAccountID == "" || info.Userinfo.Sub == serviceAccountID) {
		...
		// decision explicitly — otherwise the front-door audit logs this
