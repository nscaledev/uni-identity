...
// as a designed follow-up to the remote decision path (see "The
// Decision-Engine Crossing"). A
...
type RemoteEngine struct {
	...
	// decisions and latency are the caller-side decision instruments
	// (see metrics.go): the consumer's own view of the CheckMany round trip,
	// distinct from identity's server-side decision instruments.  Both are no-op
...
//
// This is the caller-side telemetry choke point (see metrics.go): the
...
//
// It carries no caller-side instrumentation of its own: delegating to
