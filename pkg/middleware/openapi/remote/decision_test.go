...
// These tests pin the remote authorizer's decision call:
...
func checkAuthContext(t *testing.T, token string, impersonate bool) context.Context {
	...
	p := &principal.Principal{Actor: "actor@example.com", Issuer: "https://issuer.example.com/", Type: identityapi.User, OrganizationIDs: []string{"org-1"}}
...
// the dimension impersonation is triggered from (an mTLS system-account
...
func TestRemoteCheckManyForwardsPrincipal(t *testing.T) {
	...
	raw, err := base64.RawURLEncoding.DecodeString(h.gotPrincipal)
	require.NoError(t, err)

	var propagated principal.Principal

	require.NoError(t, json.Unmarshal(raw, &propagated))
	require.Equal(t, "https://issuer.example.com/", propagated.Issuer, "the authenticating issuer must ride with the actor")
}
