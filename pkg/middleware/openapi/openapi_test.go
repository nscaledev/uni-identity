...
func TestUserToServiceAuthenticationPropagatesIssuer(t *testing.T) {
	t.Parallel()
	...
	info := authInfoFixture(userActor, identityapi.User)
	info.SrcIss = issuer

	authorizer := mock.NewMockAuthorizer(c)
	authorizer.EXPECT().Authorize(gomock.Any()).Return(info, nil)
	...
	require.NotNil(t, h.principal)
	require.Equal(t, issuer, h.principal.Issuer, "the issuer established by bearer authentication must survive principal generation")
}
...
// TestServiceToServiceForgedPrincipalWithoutMTLSIsNotHonored is the
...
// TestForgedPrincipalHeaderWithoutVerifiedPeerRejected is the trust-boundary
