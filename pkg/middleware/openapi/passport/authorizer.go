...
func (a *Authorizer) Authorize(input *openapi3filter.AuthenticationInput) (*authorization.Info, error) {
	...
	if err != nil {
		...
		default:
			...
			// Fail closed for non-availability exchange errors.
			//
			// ErrTokenExchangeFailed represents exchange responses that are neither
			// successful nor explicitly classified as availability failures
			// (for example 4xx such as 429, 400, 403). We do not fall back to
			// the legacy path for these cases to avoid masking policy/protocol
			// errors as transport degradation.
