...
func classifyTokenExchangeStatus(statusCode int) error {
	...
	case statusCode != http.StatusOK:
		// Non-200 responses that are not 401 or 5xx are classified as failed,
		// not unavailable, so callers can keep fail-closed behavior instead of
		// treating protocol/policy responses as transport degradation.
