...
Route omission is the first guard: for the `authorization` profile, every write route is absent
from the mux, so a request to one gets a 404, not a refused write. The second layer is a read-only
...
- The authenticated `src_iss` is copied into that principal and propagated with the actor. An older
  header may omit it, but the remote side keeps the resulting empty value fail-closed for global
  binding matching rather than defaulting it to the UNI sentinel.
...
- ACL cache keys must distinguish direct calls from impersonated calls so cached results do not
  overgrant. Keys are also qualified by the authenticated issuer (`src_iss`), or by the propagated
  principal issuer for the impersonated side of a service call, and every
