...
type Options struct {
	...
	// key+value bytes exceed it is rejected before compilation and causes any
	// published grants to be withdrawn (with a PolicyStoreTooLarge event)
	// instead of failing an opaque publish.  The whole store lives in one
	// ConfigMap, which the API server caps at ~1 MiB (the etcd request-size
	// limit); 0 selects that
