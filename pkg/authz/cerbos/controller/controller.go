...
// the identity namespace (pkg/authz/cerbos/generate), withdraws published
// grants when that authoritative store is deterministically rejected, and
// publishes accepted stores under content-hash-suffixed ConfigMap keys so the
// sidecar's directory watcher actually sees every change.  See the package
...
const (
	...
	// storeVersionKey is the ConfigMap key carrying the publication marker.
	// The Cerbos disk index skips an entry that is hidden or whose extension
	// is not .yaml/.yml/.json (internal/util.IsHidden and
	// IsSupportedFileType, v0.53.0), so this key is invisible to the PDP and
	// can never be read as a policy document.
	storeVersionKey = ".store-version"

	// storeStateValid and storeStateWithdrawn are the two publication states
	// the marker records.  Valid means the candidate passed every gate, even
	// if it legitimately produced no policies at all; withdrawn means a
	// deterministic rejection, which publishes no policy document.
...
// configured ConfigMap size ceiling (Reconciler.maxStoreBytes).  The candidate
// is refused before compilation and published grants are withdrawn; the
...
// errPolicyGeneration marks failures deterministically derived from a
// successfully listed Role set.  Role-list failures do not wrap it: without an
// authoritative input snapshot, replacing last-good is unsafe.
var errPolicyGeneration = errors.New("policy store generation failed")
...
// content is deliberately ignored.  Deterministic rejection of the current
// Role set publishes an empty deny-all store and still returns the rejection;
// transient read, gate-infrastructure and publish failures retain last-good.
...
func (r *Reconciler) Reconcile(ctx context.Context, _ reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		if errors.Is(err, errPolicyGeneration) {
			return reconcile.Result{}, r.withdrawRejectedStore(ctx, roles, eventPolicyStoreRejected, err)
		}

		return reconcile.Result{}, err
	}

	// The marker joins the data before the currency comparison and the size
	// accounting, so both see exactly the bytes that will be published.  It
	// also reaches compileCheck's scratch directory, where Cerbos ignores it.
	data[storeVersionKey] = storeVersion(storeStateValid)
	...
	if size := policyStoreSize(data); size > r.maxStoreBytes {
		...
		// not freeze last-good behind an opaque publish error.
		err := fmt.Errorf("%w: %d bytes exceeds %d", ErrPolicyStoreTooLarge, size, r.maxStoreBytes)

		return reconcile.Result{}, r.withdrawRejectedStore(ctx, roles, eventPolicyStoreTooLarge, err)
	}

	if err := r.compileCheck(ctx, data); err != nil {
		if errors.Is(err, ErrCompileFailed) || errors.Is(err, ErrTestsFailed) {
			return reconcile.Result{}, r.withdrawRejectedStore(ctx, roles, eventPolicyStoreRejected, err)
		}

		// An inconclusive gate says nothing authoritative about the
		// candidate.  Retain last-good until the infrastructure recovers.
		...
		r.recorder.Eventf(r.configMapRef(), corev1.EventTypeWarning, eventPolicyStoreRejected,
			"Unable to validate generated policy store, keeping last-good policies: %v", err)
	...
	logger.Info("published policy store", "operation", result, "roles", roles, "files", policyFileCount(data))
...
// A Role the generator rejects (or an unmarshalable document) is marked as a
// deterministic failure so Reconcile can withdraw any previously published
// grants.  A Role-list failure is transient and deliberately remains unmarked.
func (r *Reconciler) generateStore(ctx context.Context) (int, map[string]string, error) {
	...
	if err != nil {
		return len(roles.Items), nil, fmt.Errorf("%w: generating policies: %w", errPolicyGeneration, err)
	}

	files, err := output.Files()
	if err != nil {
		return len(roles.Items), nil, fmt.Errorf("%w: serializing policies: %w", errPolicyGeneration, err)
	}

	return len(roles.Items), hashKeyedData(files), nil
}

// withdrawRejectedStore replaces any previously published grants with the
// known-valid empty store.  The deterministic rejection remains observable as
// the returned error and a warning event.  If publication itself fails, both
// errors are returned and last-good may remain until infrastructure recovers.
func (r *Reconciler) withdrawRejectedStore(ctx context.Context, roles int, reason string, rejection error) error {
	logger := log.FromContext(ctx)

	result, err := r.publish(ctx, withdrawnStore())
	if err != nil {
		publishErr := fmt.Errorf("withdrawing rejected policy store: %w", err)
		logger.Error(rejection, "rejected policy store but failed to withdraw published policies", "roles", roles, "publishError", publishErr)
		r.recorder.Eventf(r.configMapRef(), corev1.EventTypeWarning, reason,
			"Rejected generated policy store but failed to withdraw published policies: %v; %v", rejection, publishErr)

		return errors.Join(rejection, publishErr)
	}

	logger.Error(rejection, "rejected policy store and withdrew published policies", "operation", result, "roles", roles)
	r.recorder.Eventf(r.configMapRef(), corev1.EventTypeWarning, reason,
		"Rejected generated policy store and withdrew all published policies: %v", rejection)
...
func (r *Reconciler) storeIsCurrent(ctx context.Context, data map[string]string) (bool, error) {
	...
	if err := r.client.Get(ctx, types.NamespacedName{Namespace: r.namespace, Name: r.configMapName}, existing); err != nil {
		...
		// (this also absorbs the one-time upgrade from the Helm-owned
		// empty ConfigMap to the controller-written store, which deletes
...
// storeIsCurrent stays a plain comparison.  A monotonic revision would need a
// read-modify-write plus a rule to suppress the republish loop that creates,
// and nothing reads a revision today: a pod has no independent source telling
// it which revision ought to be current.
func storeVersion(state string) string {
	return fmt.Sprintf("{\"schema\":1,\"state\":%q}\n", state)
}

// withdrawnStore is the complete ConfigMap data of a withdrawn publication:
// the marker, and no policy document at all.  "No PDP-visible policy" is the
// security invariant here, not "no ConfigMap key".
func withdrawnStore() map[string]string {
	return map[string]string{storeVersionKey: storeVersion(storeStateWithdrawn)}
}

// policyFileCount counts the PDP-visible documents in a candidate store.  The
// marker is not one, so a store that generated no policies still counts zero
// and is recognised as having nothing to compile.
func policyFileCount(data map[string]string) int {
	count := 0

	for name := range data {
		if name != storeVersionKey {
			count++
		}
	}
