...
// same ExecGate code path and the same pinned compiler, matching the Cerbos
// client integration test's docker-run conventions.
package controller
...
// falling back to a hardcoded image tag keeps the version pin out of this
// test: make test-cerbos-controller passes the binary for CERBOS_VERSION.
...
// small role set: generate.Generate output under hash-suffixed keys, plus the
// publication marker.
