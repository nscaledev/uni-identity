...
// These tests pin the policy-store hasher: a read-through fingerprint of
...
func TestPolicyStoreHasherServesLastGoodDuringSlowRefresh(t *testing.T) {
	...
	// A short positive interval makes a refresh due promptly without relying
	// on the invalid zero interval production normalizes.
	hasher := cerbos.NewPolicyStoreHasher(counting, hashNamespace, hashConfigMap, time.Millisecond)
	...
	require.Eventually(t, func() bool {
		_, _ = currentPrompt(t, hasher)

		return counting.count() > base
	}, 10*time.Second, time.Millisecond, "a due refresh must start the gated ConfigMap read")
...
func TestPolicyStoreHasherRefreshSurvivesCallerCancellation(t *testing.T) {
	...
	// whole process (cache bypassed and audit correlate lost process-wide until
	// the next interval).
	counting := &countingClient{Client: hashClient(t, policyConfigMap("identity_groups-0a1b2c3d.yaml"))}
...
// TestPolicyStoreHasherNormalizesNonPositiveRefresh pins the production safety
// boundary: an unset or invalid interval must not let per-decision Current
// calls drive one Kubernetes GET each after the preceding read completes.
func TestPolicyStoreHasherNormalizesNonPositiveRefresh(t *testing.T) {
	t.Parallel()

	for _, refresh := range []time.Duration{0, -time.Second} {
		t.Run(refresh.String(), func(t *testing.T) {
			t.Parallel()

			counting := &countingClient{Client: hashClient(t, policyConfigMap("identity_groups-0123456789abcdef0123456789abcdef.yaml"))}
			hasher := cerbos.NewPolicyStoreHasher(counting, hashNamespace, hashConfigMap, refresh)
			...
			require.Equal(t, 1, counting.count(), "non-positive refresh %v must be normalized instead of re-reading per call", refresh)
		})
	}
}
...
func TestPolicyStoreHasherKeySetChangeChangesHash(t *testing.T) {
	...
	// A short positive interval lets the test observe a republish without
	// relying on the invalid zero interval production normalizes.
	hasher := cerbos.NewPolicyStoreHasher(c, hashNamespace, hashConfigMap, time.Millisecond)
...
func TestPolicyStoreHasherRetainsLastGoodAfterFailedRefresh(t *testing.T) {
	...
	// A short positive interval lets the test trigger a failing refresh without
	// relying on the invalid zero interval production normalizes.
	hasher := cerbos.NewPolicyStoreHasher(counting, hashNamespace, hashConfigMap, time.Millisecond)
