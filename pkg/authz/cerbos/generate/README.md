...
[design note](../../../../docs/authorization/cerbos-authorization-design.md) for where this fits in the broader
...
verbatim. Existing sanitized names are retained when unique and at most 253 bytes. Names that collide
after sanitization (including case-only collisions) or exceed that ConfigMap key/filesystem component
limit receive a deterministic SHA-256 suffix and a bounded readable prefix, so every endpoint remains
a distinct policy file.
...
- Policy file names are valid ConfigMap keys, safe filesystem components, and at most 253 bytes.
...
fixtures. `TestFixturesMatchChart` fails when a chart role has no fixture, or when a fixture's grants
differ from the chart after that placeholder mapping. When the chart's roles change, update the
fixtures and regenerate the golden store with `go test ./pkg/authz/cerbos/generate/... -update`.
