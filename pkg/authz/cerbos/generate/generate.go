...
const (
	...
	// maxPolicyFileNameLength is both the Kubernetes ConfigMap key limit and
	// the maximum portable component length of the generated policy store.
	maxPolicyFileNameLength = 253
)
...
// policyFileName returns the preferred store-relative file name for an
// endpoint's resource policy. Endpoint names may contain characters that are
// unsafe in file names (e.g. '/'), so anything outside [a-zA-Z0-9._-] becomes
// '_'. policyFileNames disambiguates collisions and bounds long names.
...
// policyFileNames assigns a distinct ConfigMap/filesystem-safe name to every
// endpoint.  A sanitized name is retained when it is unique on a
// case-insensitive filesystem and within the length limit, so the normal
// store keeps readable names.  Every other endpoint takes the SHA-256 of its
// own (verbatim, unsanitized) name as the suffix, which is unique per
// endpoint by construction, so two disambiguated names cannot collide; the
// readable prefix is truncated to whatever the limit leaves.
//
// The result depends only on the input set, never on iteration order.
func policyFileNames(endpoints []string) map[string]string {
	preferred := make(map[string]string, len(endpoints))
	...
	for _, endpoint := range endpoints {
		...
		if len(name) <= maxPolicyFileNameLength && counts[strings.ToLower(name)] == 1 {
			names[endpoint] = name

			continue
		}

		sanitized := strings.TrimSuffix(strings.TrimPrefix(name, "resource_"), ".yaml")
		suffix := fmt.Sprintf("_%x", sha256.Sum256([]byte(endpoint))) + ".yaml"
		prefixLength := maxPolicyFileNameLength - len("resource_") - len(suffix)

		names[endpoint] = "resource_" + sanitized[:min(len(sanitized), prefixLength)] + suffix
	}
...
func (o *Output) Files() (map[string][]byte, error) {
	...
	endpoints := make([]string, len(o.ResourcePolicies))

	for i, document := range o.ResourcePolicies {
		endpoints[i] = document.ResourcePolicy.Resource
	}

	fileNames := policyFileNames(endpoints)
	...
	for _, document := range o.ResourcePolicies {
		...
		files[fileNames[document.ResourcePolicy.Resource]] = data
	}
