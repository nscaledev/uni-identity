/*
Copyright 2026 Nscale.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package generate_test

import (
	"flag"
	"maps"
	"os"
	"path/filepath"
	"slices"
	"strings"
	"testing"

	"github.com/stretchr/testify/require"

	"github.com/unikorn-cloud/core/pkg/constants"
	unikornv1 "github.com/unikorn-cloud/identity/pkg/apis/unikorn/v1alpha1"
	"github.com/unikorn-cloud/identity/pkg/authz/cerbos/generate"

	"k8s.io/apimachinery/pkg/util/validation"

	"sigs.k8s.io/yaml"
)

// update regenerates the golden policy store from the fixture roles instead
// of comparing against it: go test ./pkg/authz/cerbos/generate/... -update
//
//nolint:gochecknoglobals // Standard golden-file -update flag pattern.
var update = flag.Bool("update", false, "regenerate golden files")

const (
	rolesDir    = "testdata/roles"
	storeDir    = "testdata/store"
	chartValues = "../../../../charts/identity/values.yaml"
)

// chartPlaceholders maps the chart names that the fixtures replace with
// generic placeholders (see "Fixtures and provenance" in the README).
//
//nolint:gochecknoglobals // Read-only lookup table.
var chartPlaceholders = map[string]string{
	"storage-service":                           "blob-service",
	"storage:objectstorageclasses":              "blob:buckets",
	"storage:objectstorageendpoints":            "blob:stores",
	"storage:objectstorageendpoints/accesskeys": "blob:stores/keys",
}

func placeholder(name string) string {
	if replacement, ok := chartPlaceholders[name]; ok {
		return replacement
	}

	return name
}

// loadFixtureRoles reads every Role CR fixture committed under
// testdata/roles.  The fixtures transcribe every built-in role from
// charts/identity/values.yaml (TestFixturesMatchChart keeps them in step)
// plus representative out-of-repo open-vocabulary roles and synthetic edge
// cases; see the per-file provenance comments.
func loadFixtureRoles(t *testing.T) []unikornv1.Role {
	t.Helper()

	paths, err := filepath.Glob(filepath.Join(rolesDir, "*.yaml"))
	require.NoError(t, err)
	require.NotEmpty(t, paths)

	roles := make([]unikornv1.Role, 0, len(paths))

	for _, path := range paths {
		data, err := os.ReadFile(path)
		require.NoError(t, err)

		var role unikornv1.Role

		require.NoError(t, yaml.UnmarshalStrict(data, &role), path)
		roles = append(roles, role)
	}

	return roles
}

// readStoreFiles returns the generated files committed at the top level of
// the golden store.  The tests/ subdirectory holds the hand-written Cerbos
// test suite and is not generator output.
func readStoreFiles(t *testing.T) map[string][]byte {
	t.Helper()

	entries, err := os.ReadDir(storeDir)
	require.NoError(t, err)

	files := map[string][]byte{}

	for _, entry := range entries {
		if entry.IsDir() || !strings.HasSuffix(entry.Name(), ".yaml") {
			continue
		}

		data, err := os.ReadFile(filepath.Join(storeDir, entry.Name()))
		require.NoError(t, err)

		files[entry.Name()] = data
	}

	return files
}

// writeStoreFiles replaces the generated top level of the golden store,
// leaving the hand-written tests/ subdirectory untouched.
func writeStoreFiles(t *testing.T, files map[string][]byte) {
	t.Helper()

	for name := range readStoreFiles(t) {
		require.NoError(t, os.Remove(filepath.Join(storeDir, name)))
	}

	for name, data := range files {
		require.NoError(t, os.WriteFile(filepath.Join(storeDir, name), data, 0o600))
	}
}

// role is a convenience constructor for the edge-case tests.
func role(id string, scopes unikornv1.RoleScopes) unikornv1.Role {
	out := unikornv1.Role{}
	out.Name = id
	out.Spec.Scopes = scopes

	return out
}

func scope(name string, operations ...unikornv1.Operation) unikornv1.RoleScope {
	return unikornv1.RoleScope{Name: name, Operations: operations}
}

// TestGenerateGolden pins the full byte-exact output for the fixture roles.
// The golden store doubles as the Cerbos compile store validated by
// `make validate-policies`, so any change to these bytes must keep that
// target passing: determinism and the binding-string contract are enforced
// here, behavioural semantics in the Cerbos test suite.
func TestGenerateGolden(t *testing.T) {
	t.Parallel()

	output, err := generate.Generate(loadFixtureRoles(t))
	require.NoError(t, err)

	files, err := output.Files()
	require.NoError(t, err)

	if *update {
		writeStoreFiles(t, files)
		return
	}

	golden := readStoreFiles(t)

	require.Equal(t, slices.Sorted(maps.Keys(golden)), slices.Sorted(maps.Keys(files)),
		"generated file set differs from golden store; run: go test ./pkg/authz/cerbos/generate/... -update")

	for name, want := range golden {
		require.Equal(t, string(want), string(files[name]), name)
	}
}

// grantSet flattens a role's grants to "<scope> <endpoint>" keys with sorted,
// unique operations, so that a chart role and a fixture compare as sets.
type grantSet map[string][]string

func (g grantSet) add(scope, endpoint string, operations ...string) {
	if len(operations) == 0 {
		return
	}

	key := scope + " " + endpoint

	g[key] = append(g[key], operations...)
	slices.Sort(g[key])
	g[key] = slices.Compact(g[key])
}

// TestFixturesMatchChart makes sure that every role in
// charts/identity/values.yaml has a fixture with the same grants.  The golden
// store is generated from the fixtures, so a fixture that drifts from the
// chart pins a store that no deployment produces, and the golden and Cerbos
// tests then pass against grants that nobody ships.
func TestFixturesMatchChart(t *testing.T) {
	t.Parallel()

	data, err := os.ReadFile(chartValues)
	require.NoError(t, err)

	var values struct {
		Roles map[string]struct {
			Scopes map[string]map[string][]string `json:"scopes"`
		} `json:"roles"`
	}

	require.NoError(t, yaml.Unmarshal(data, &values))
	require.NotEmpty(t, values.Roles)

	fixtures := map[string]unikornv1.RoleScopes{}

	for _, role := range loadFixtureRoles(t) {
		fixtures[role.Labels[constants.NameLabel]] = role.Spec.Scopes
	}

	for name, chartRole := range values.Roles {
		fixture, ok := fixtures[placeholder(name)]
		require.True(t, ok, "chart role %q has no fixture under %s", name, rolesDir)

		want := grantSet{}

		for scope, endpoints := range chartRole.Scopes {
			for endpoint, operations := range endpoints {
				want.add(scope, placeholder(endpoint), operations...)
			}
		}

		got := grantSet{}

		for scope, entries := range map[string][]unikornv1.RoleScope{
			"global":       fixture.Global,
			"organization": fixture.Organization,
			"project":      fixture.Project,
		} {
			for _, entry := range entries {
				for _, operation := range entry.Operations {
					got.add(scope, entry.Name, string(operation))
				}
			}
		}

		require.Equal(t, want, got,
			"the fixture for chart role %q drifts from the chart; update %s, then run: go test ./pkg/authz/cerbos/generate/... -update", name, rolesDir)
	}
}

// TestGenerateGrantsNothing verifies that inputs which grant nothing produce
// an empty store: emitting an empty derived-roles document or a resource
// policy with no rules would be rejected by Cerbos.
func TestGenerateGrantsNothing(t *testing.T) {
	t.Parallel()

	cases := map[string][]unikornv1.Role{
		"NoRoles": nil,
		"NilScopes": {
			role("00000000-0000-0000-0000-000000000001", unikornv1.RoleScopes{}),
		},
		"EmptyBuckets": {
			role("00000000-0000-0000-0000-000000000001", unikornv1.RoleScopes{
				Global:       []unikornv1.RoleScope{},
				Organization: []unikornv1.RoleScope{},
				Project:      []unikornv1.RoleScope{},
			}),
		},
		"NoOperations": {
			role("00000000-0000-0000-0000-000000000001", unikornv1.RoleScopes{
				Organization: []unikornv1.RoleScope{scope("identity:groups")},
			}),
		},
	}

	for name, roles := range cases {
		t.Run(name, func(t *testing.T) {
			t.Parallel()

			output, err := generate.Generate(roles)
			require.NoError(t, err)

			require.Nil(t, output.DerivedRoles)
			require.Empty(t, output.ResourcePolicies)

			files, err := output.Files()
			require.NoError(t, err)
			require.Empty(t, files)
		})
	}
}

// TestGenerateUnionAndDedup pins the byte-exact resource policy for two roles
// granting overlapping operations on one endpoint: the grants union as
// separate rules (Cerbos ALLOWs are additive, mirroring pkg/rbac's additive
// roles), and duplicate operations within one scope dedup into sorted
// actions.
func TestGenerateUnionAndDedup(t *testing.T) {
	t.Parallel()

	roles := []unikornv1.Role{
		role("00000000-0000-0000-0000-00000000000b", unikornv1.RoleScopes{
			Organization: []unikornv1.RoleScope{
				scope("example:widgets", unikornv1.Read, unikornv1.Read, unikornv1.Create),
			},
		}),
		role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
			Global: []unikornv1.RoleScope{
				scope("example:widgets", unikornv1.Update),
			},
		}),
	}

	output, err := generate.Generate(roles)
	require.NoError(t, err)

	files, err := output.Files()
	require.NoError(t, err)
	require.Len(t, files, 2)

	expected := `# Code generated by github.com/unikorn-cloud/identity/pkg/authz/cerbos/generate. DO NOT EDIT.
apiVersion: api.cerbos.dev/v1
resourcePolicy:
  resource: example:widgets
  version: default
  scope: ""
  scopePermissions: SCOPE_PERMISSIONS_OVERRIDE_PARENT
  importDerivedRoles: [uni_roles]
  rules:
    - actions: [update]
      derivedRoles: [role_00000000-0000-0000-0000-00000000000a_global]
      effect: EFFECT_ALLOW
    - actions: [create, read]
      derivedRoles: [role_00000000-0000-0000-0000-00000000000b_org]
      effect: EFFECT_ALLOW
`

	require.Equal(t, expected, string(files["resource_example_widgets.yaml"]))
}

// TestGenerateGlobalReadBucket pins the byte-exact read-only projection of a
// role's global block.  The clamp is what a wildcard subject binding gets: the
// legacy ACL path grants read on every global scope that has read and drops
// every scope that does not (pkg/rbac accumulateGlobalReadPermissions), so the
// projected bucket must keep example:widgets at read alone and omit
// example:gadgets entirely.  The full global bucket stays beside it, because an
// exact or group binding still activates it.
func TestGenerateGlobalReadBucket(t *testing.T) {
	t.Parallel()

	roles := []unikornv1.Role{
		role("00000000-0000-0000-0000-00000000000c", unikornv1.RoleScopes{
			Global: []unikornv1.RoleScope{
				scope("example:widgets", unikornv1.Create, unikornv1.Read),
				scope("example:gadgets", unikornv1.Delete),
			},
		}),
	}

	output, err := generate.Generate(roles)
	require.NoError(t, err)

	files, err := output.Files()
	require.NoError(t, err)
	require.Len(t, files, 3)

	expectedDerivedRoles := `# Code generated by github.com/unikorn-cloud/identity/pkg/authz/cerbos/generate. DO NOT EDIT.
apiVersion: api.cerbos.dev/v1
derivedRoles:
  name: uni_roles
  definitions:
    - name: role_00000000-0000-0000-0000-00000000000c_global
      parentRoles: [principal]
      condition:
        match:
          expr: '"00000000-0000-0000-0000-00000000000c#global" in P.attr.bindings'
    - name: role_00000000-0000-0000-0000-00000000000c_global-read
      parentRoles: [principal]
      condition:
        match:
          expr: '"00000000-0000-0000-0000-00000000000c#global-read" in P.attr.bindings'
`

	require.Equal(t, expectedDerivedRoles, string(files["derived_roles.yaml"]))

	expectedWidgets := `# Code generated by github.com/unikorn-cloud/identity/pkg/authz/cerbos/generate. DO NOT EDIT.
apiVersion: api.cerbos.dev/v1
resourcePolicy:
  resource: example:widgets
  version: default
  scope: ""
  scopePermissions: SCOPE_PERMISSIONS_OVERRIDE_PARENT
  importDerivedRoles: [uni_roles]
  rules:
    - actions: [create, read]
      derivedRoles: [role_00000000-0000-0000-0000-00000000000c_global]
      effect: EFFECT_ALLOW
    - actions: [read]
      derivedRoles: [role_00000000-0000-0000-0000-00000000000c_global-read]
      effect: EFFECT_ALLOW
`

	require.Equal(t, expectedWidgets, string(files["resource_example_widgets.yaml"]))

	expectedGadgets := `# Code generated by github.com/unikorn-cloud/identity/pkg/authz/cerbos/generate. DO NOT EDIT.
apiVersion: api.cerbos.dev/v1
resourcePolicy:
  resource: example:gadgets
  version: default
  scope: ""
  scopePermissions: SCOPE_PERMISSIONS_OVERRIDE_PARENT
  importDerivedRoles: [uni_roles]
  rules:
    - actions: [delete]
      derivedRoles: [role_00000000-0000-0000-0000-00000000000c_global]
      effect: EFFECT_ALLOW
`

	require.Equal(t, expectedGadgets, string(files["resource_example_gadgets.yaml"]))
}

// TestGenerateGlobalReadBucketOmittedWithoutRead verifies that a role whose
// global block grants no read operation gets no read-only bucket at all.  A
// wildcard binding on such a role must activate nothing, which is what the
// legacy clamp computes: an empty scope set, not a fallback to the full bucket.
//
// Both role shapes share one input on purpose.  An absence assertion alone
// would also hold if the bucket were never emitted for any role, so the
// readable role pins the presence half in the same breath as the absence half.
func TestGenerateGlobalReadBucketOmittedWithoutRead(t *testing.T) {
	t.Parallel()

	roles := []unikornv1.Role{
		role("00000000-0000-0000-0000-00000000000d", unikornv1.RoleScopes{
			Global: []unikornv1.RoleScope{
				scope("example:widgets", unikornv1.Create, unikornv1.Update, unikornv1.Delete),
			},
		}),
		role("00000000-0000-0000-0000-00000000000e", unikornv1.RoleScopes{
			Global: []unikornv1.RoleScope{
				scope("example:gadgets", unikornv1.Read),
			},
		}),
	}

	output, err := generate.Generate(roles)
	require.NoError(t, err)
	require.NotNil(t, output.DerivedRoles)

	names := make([]string, 0, len(output.DerivedRoles.DerivedRoles.Definitions))

	for _, definition := range output.DerivedRoles.DerivedRoles.Definitions {
		names = append(names, definition.Name)
	}

	require.Equal(t, []string{
		"role_00000000-0000-0000-0000-00000000000d_global",
		"role_00000000-0000-0000-0000-00000000000e_global",
		"role_00000000-0000-0000-0000-00000000000e_global-read",
	}, names)
}

// TestGenerateMergesDuplicateEndpoints verifies that duplicate endpoint names
// within one bucket union their operations into a single rule, mirroring
// pkg/rbac's additive-union semantics.
func TestGenerateMergesDuplicateEndpoints(t *testing.T) {
	t.Parallel()

	roles := []unikornv1.Role{
		role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
			Organization: []unikornv1.RoleScope{
				scope("example:widgets", unikornv1.Read),
				scope("example:widgets", unikornv1.Create, unikornv1.Read),
			},
		}),
	}

	output, err := generate.Generate(roles)
	require.NoError(t, err)

	require.Len(t, output.ResourcePolicies, 1)

	rules := output.ResourcePolicies[0].ResourcePolicy.Rules
	require.Len(t, rules, 1)
	require.Equal(t, []string{"create", "read"}, rules[0].Actions)
}

// TestGenerateIsOrderInvariant verifies byte-stable output regardless of
// input order: the store is compared byte-for-byte by controllers and golden
// tests, so ordering must come from the data, not the caller.
func TestGenerateIsOrderInvariant(t *testing.T) {
	t.Parallel()

	roles := loadFixtureRoles(t)

	forward, err := generate.Generate(roles)
	require.NoError(t, err)

	slices.Reverse(roles)

	reversed, err := generate.Generate(roles)
	require.NoError(t, err)

	forwardFiles, err := forward.Files()
	require.NoError(t, err)

	reversedFiles, err := reversed.Files()
	require.NoError(t, err)

	require.Len(t, reversedFiles, len(forwardFiles))

	for name, data := range forwardFiles {
		require.Equal(t, string(data), string(reversedFiles[name]), name)
	}
}

// TestGenerateInvalidInput verifies the generator fails loudly on inputs the
// binding-string contract cannot represent, rather than emitting policies
// that are broken or ambiguous.
func TestGenerateInvalidInput(t *testing.T) {
	t.Parallel()

	grants := unikornv1.RoleScopes{
		Organization: []unikornv1.RoleScope{scope("identity:groups", unikornv1.Read)},
	}

	cases := map[string]struct {
		roles []unikornv1.Role
		err   error
	}{
		"EmptyRoleID": {
			roles: []unikornv1.Role{role("", grants)},
			err:   generate.ErrInvalidRole,
		},
		"RoleIDNotAKubernetesName": {
			roles: []unikornv1.Role{role("role#1", grants)},
			err:   generate.ErrInvalidRole,
		},
		"DuplicateRoleIDs": {
			roles: []unikornv1.Role{
				role("00000000-0000-0000-0000-00000000000a", grants),
				role("00000000-0000-0000-0000-00000000000a", grants),
			},
			err: generate.ErrInvalidRole,
		},
		"EmptyEndpointNameWithOperations": {
			roles: []unikornv1.Role{
				role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
					Organization: []unikornv1.RoleScope{scope("", unikornv1.Read)},
				}),
			},
			err: generate.ErrInvalidScope,
		},
		"EndpointNameWithGlobAsterisk": {
			roles: []unikornv1.Role{
				role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
					Organization: []unikornv1.RoleScope{scope("example:*", unikornv1.Read)},
				}),
			},
			err: generate.ErrInvalidScope,
		},
		"EndpointNameWithGlobQuestionMark": {
			roles: []unikornv1.Role{
				role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
					Organization: []unikornv1.RoleScope{scope("example:widgets?", unikornv1.Read)},
				}),
			},
			err: generate.ErrInvalidScope,
		},
		"EndpointNameWithGlobBrackets": {
			roles: []unikornv1.Role{
				role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
					Organization: []unikornv1.RoleScope{scope("example:widgets[1]", unikornv1.Read)},
				}),
			},
			err: generate.ErrInvalidScope,
		},
		"EndpointNameWithGlobBraces": {
			roles: []unikornv1.Role{
				role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
					Organization: []unikornv1.RoleScope{scope("example:{widgets}", unikornv1.Read)},
				}),
			},
			err: generate.ErrInvalidScope,
		},
		"EndpointNameWithGlobExclamationMark": {
			roles: []unikornv1.Role{
				role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
					Organization: []unikornv1.RoleScope{scope("example:widgets!", unikornv1.Read)},
				}),
			},
			err: generate.ErrInvalidScope,
		},
	}

	for name, testcase := range cases {
		t.Run(name, func(t *testing.T) {
			t.Parallel()

			_, err := generate.Generate(testcase.roles)
			require.ErrorIs(t, err, testcase.err)
		})
	}
}

// TestGenerateDisambiguatesPolicyFileNames verifies that lossy sanitization
// and case-insensitive filesystems cannot make one valid endpoint overwrite
// another endpoint's policy.
func TestGenerateDisambiguatesPolicyFileNames(t *testing.T) {
	t.Parallel()

	endpoints := []string{"example/widgets", "example:widgets", "example:widgetS"}
	roles := []unikornv1.Role{
		role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
			Organization: []unikornv1.RoleScope{
				scope(endpoints[0], unikornv1.Read),
				scope(endpoints[1], unikornv1.Read),
				scope(endpoints[2], unikornv1.Read),
			},
		}),
	}

	output, err := generate.Generate(roles)
	require.NoError(t, err)

	files, err := output.Files()
	require.NoError(t, err)
	require.Len(t, files, len(endpoints)+1, "each endpoint policy must survive serialization")

	resources := map[string]string{}
	fileNames := map[string]bool{}

	for name, data := range files {
		if name == "derived_roles.yaml" {
			continue
		}

		require.Empty(t, validation.IsConfigMapKey(name), name)
		require.LessOrEqual(t, len(name), 253, name)
		require.False(t, fileNames[strings.ToLower(name)], "policy file names must also be distinct on case-insensitive filesystems")
		fileNames[strings.ToLower(name)] = true

		var document generate.ResourcePolicyDocument

		require.NoError(t, yaml.UnmarshalStrict(data, &document), name)
		resources[document.ResourcePolicy.Resource] = name
	}

	require.ElementsMatch(t, endpoints, slices.Collect(maps.Keys(resources)))
}

// TestGenerateBoundsLongPolicyFileNames verifies that endpoint vocabulary is
// not constrained by the ConfigMap key limit: the filename is bounded while
// the policy's resource kind remains verbatim.
func TestGenerateBoundsLongPolicyFileNames(t *testing.T) {
	t.Parallel()

	endpoint := "example:" + strings.Repeat("long-segment-", 30)
	roles := []unikornv1.Role{
		role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
			Organization: []unikornv1.RoleScope{scope(endpoint, unikornv1.Read)},
		}),
	}

	output, err := generate.Generate(roles)
	require.NoError(t, err)

	files, err := output.Files()
	require.NoError(t, err)
	require.Len(t, files, 2)

	for name, data := range files {
		if name == "derived_roles.yaml" {
			continue
		}

		require.Empty(t, validation.IsConfigMapKey(name), name)
		require.LessOrEqual(t, len(name), 253, name)

		var document generate.ResourcePolicyDocument

		require.NoError(t, yaml.UnmarshalStrict(data, &document))
		require.Equal(t, endpoint, document.ResourcePolicy.Resource)
	}
}

// TestGenerateEndpointNameValidationIsNotStricterThanCerbos pins the
// endpoint-name check to what Cerbos v0.53.0 actually loads: only glob
// metacharacters (and the empty string) are rejected, so names with a leading
// digit, a space or a '#' — which older Cerbos releases refused — must
// generate cleanly.  Endpoint names are an open vocabulary, so the generator
// must never reject a name the pinned Cerbos version accepts.
func TestGenerateEndpointNameValidationIsNotStricterThanCerbos(t *testing.T) {
	t.Parallel()

	roles := []unikornv1.Role{
		role("00000000-0000-0000-0000-00000000000a", unikornv1.RoleScopes{
			Organization: []unikornv1.RoleScope{
				scope("0day:reports", unikornv1.Read),
				scope("example:widget gadget", unikornv1.Read),
				scope("example:widgets#detail", unikornv1.Read),
			},
		}),
	}

	output, err := generate.Generate(roles)
	require.NoError(t, err)
	require.Len(t, output.ResourcePolicies, 3)
}
