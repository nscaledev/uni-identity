...
// file under a content-addressed key (`<base>-<sha256[:32]>.yaml`, see
...
// defaultPolicyStoreRefresh is used when callers construct a hasher with an
// unset or invalid interval.  It matches the coarse-decision cache default.
const defaultPolicyStoreRefresh = time.Minute

// NewPolicyStoreHasher returns a hasher reading the ConfigMap named name in
// namespace through client, re-reading at most once per refresh interval.  A
// non-positive interval is normalized to one minute so authorization traffic
// cannot turn the read-through into one Kubernetes GET per decision.
func NewPolicyStoreHasher(client client.Client, namespace, name string, refresh time.Duration) *PolicyStoreHasher {
	if refresh <= 0 {
		refresh = defaultPolicyStoreRefresh
	}
