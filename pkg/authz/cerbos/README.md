...
remote PDP would need mTLS and revisits this), endpoints without a decimal port
in the range 1 through 65535, and non-positive check timeouts (a zero value would
insta-deny every check while the sidecar looks healthy).  Construction is
otherwise lazy: the underlying gRPC channel
...
(`ErrCompileFailed`, `ErrTestsFailed`).  These classified rejections and any
generation error are deterministic outcomes from the successfully read,
authoritative Role set.  The controller never leaves an older, potentially
wider store serving in those cases: it publishes a known-valid empty store
(deny-all), emits a warning event (`PolicyStoreRejected`), and logs and returns
the original rejection for retry with backoff.  There is no code path that
publishes an unvetted candidate, and the gate never passes `--skip-tests`.

### The publication marker

Every publish carries one extra ConfigMap key, `.store-version`, holding
`{"schema":1,"state":"valid"}` or `{"schema":1,"state":"withdrawn"}`.  A
withdrawal publishes that key and **no policy document at all**, so the
security invariant is "no PDP-visible policy", not "no ConfigMap key".
...
- **The PDP never sees it.** The Cerbos disk index skips an entry that is
  hidden or whose extension is not `.yaml`, `.yml` or `.json`
  (`internal/util.IsHidden`, `IsSupportedFileType`, v0.53.0), so the marker
  cannot be read as a policy.  The compile gate materializes it into the
  scratch directory like any other key and the real `cerbos compile` ignores
  it, which the controller integration test exercises.
- **It does not count as a policy.** `policyFileCount` ignores it, so a Role
  set that legitimately generates nothing is still a *valid* publication of no
  policies with nothing to compile, not a withdrawal.
- **It cannot cause a republish loop.** The marker is a pure function of the
  publish decision, so the desired ConfigMap data never depends on the
  observed data and `storeIsCurrent` stays a plain comparison.  A monotonic
  revision would need a read-modify-write plus a rule to suppress the loop
  that creates, and nothing reads a revision: a pod has no independent source
  telling it which revision ought to be current.

It is **not** an applied acknowledgement.  It proves the pod observed a
projection, not that Cerbos loaded it and dropped the previous generation.
The ConfigMap is a one-way channel and the marker is invisible to the PDP by
design, so proving application needs PDP-observable evidence of the loaded
generation plus a feedback channel back to the controller; neither exists
today.

Inconclusive infrastructure failures have different semantics.  An
unclassified gate exit, missing/killed compiler, scratch-directory I/O error,
Role-list failure, ConfigMap read failure, or publication failure retains
last-good because the controller either lacks an authoritative input snapshot
or cannot safely replace the store.  A gate-infrastructure failure still emits
`PolicyStoreRejected` and returns an error.  If publishing deny-all after a
deterministic rejection itself fails, the returned error contains both the
rejection and publication failure, and the warning event reports that stale
policies may remain.
...
replaced with the empty deny-all store, a warning event (`PolicyStoreTooLarge`)
is emitted on it, and an `ErrPolicyStoreTooLarge` reconcile error is returned
for backoff.  Without this gate an over-cap store fails the opaque
`CreateOrUpdate` publish and can silently freeze at a stale, wider store; the
gate turns that unsafe freeze into a legible fail-closed refusal.
...
`<base>-<sha256[:32]>.yaml` (128 bits / 32 hex characters of the content hash).
If necessary, `<base>` is truncated so the complete suffix and extension fit
ConfigMap's 253-byte key limit; the content fingerprint is never shortened.
...
- Fake-client unit tests pin the reconcile contract: byte-exact publishes
  ...
  last-good only for transient infrastructure failures, deterministic
  generation/size/compile/test rejections withdrawing formerly served grants,
  Role deletion shrinking the store (the watch predicate passing
...
- **Content-addressed off the key set.** The fingerprint is
  ...
  every key (`<base>-<sha256[:32]>.yaml`): any policy content change changes at
...
- **Why read the ConfigMap, not the PDP.** Identity cannot get this from Cerbos:
  ...
  between. Non-positive intervals are normalized to one minute, preventing
  invalid programmatic options from turning authorization traffic into one
  Kubernetes GET per decision. `Current` is concurrency-safe and never blocks.
  It is called on every cerbos-mode decision; a due call elects one background read — detached from
